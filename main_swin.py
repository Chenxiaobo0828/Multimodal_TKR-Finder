...
def main(path, savepath, target_category, weights_path, load):
    ...
    if load == 1:
        visualization = np.flip(visualization, axis=1)
...
path = r"/mnt/d/kneesoft/bulaiying.nii.gz"
motai = "COR_MPR"
label = 1

savepath = r"/mnt/d/kneesoft/testCAM"

if motai == "Xray":
    weights_path = "./weights/wXRAY.pth"
if motai == "SAG_IW_TSE":
    weights_path = "./weights/wSAG.pth"
if motai == "COR_MPR":
    weights_path = "./weights/wCOR.pth"
...
if motai == "Xray":
    thesave = os.path.join(savepath, motai, name[:-4] + ".jpg")  # jpg
    main(path, thesave, label, weights_path, load=0)
else:
    thesave = os.path.join(savepath, motai, name[:-7] + ".jpg")  # .nii.gz
